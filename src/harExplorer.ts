...
export class HarOutlineProvider implements vscode.TreeDataProvider<number> {
	...
	private tree: json.Node | null = null;
	private text: string = "";
	...
	rename(offset: number): void {
		vscode.window.showInputBox({ placeHolder: 'Enter the new label' })
			.then(value => {
				if (value !== null && value !== undefined && this.tree !== null && this.editor !== undefined) {
					this.editor.edit(editBuilder => {
						const path = json.getLocation(this.text, offset).path;
                        let propertyNode = json.findNodeAtLocation(this.tree as json.Node, path);
                        if (propertyNode !== undefined && this.editor !== undefined) {
                            if (propertyNode !== undefined && propertyNode.parent !== undefined &&
                                propertyNode.parent.type !== 'array' && propertyNode.parent.children !== undefined) {
	...
	private onDocumentChanged(changeEvent: vscode.TextDocumentChangeEvent): void {
		if (this.autoRefresh && this.editor !== undefined && changeEvent.document.uri.toString() === this.editor.document.uri.toString()) {
			for (const change of changeEvent.contentChanges) {
				const path = json.getLocation(this.text, this.editor.document.offsetAt(change.range.start)).path;
				path.pop();
				const node = path.length && this.tree !== null ? json.findNodeAtLocation(this.tree, path) : void 0;
				this.parseTree();
	...
	getChildren(offset?: number): Thenable<number[]> {
        ...
        if (offset) {
            ...
            return node === undefined ? Promise.resolve([]) : 
                Promise.resolve(this.getChildrenOffsets(node));
		} else {
	...
	private getChildrenOffsets(node: json.Node): number[] {
        if (this.tree === null || node === null || node.children === undefined) {
            return [];
        }
	...
	getTreeItem(offset: number): vscode.TreeItem {
        if (this.tree === null || this.editor === undefined) {
	...
	private getLabel(node: json.Node): string {
        if (node.parent === undefined || node.parent.children === undefined || this.editor === undefined) {
            return "";
        }
